...
function update(seq,id) {
	if(<%=udto.getId()%> != id ){
		alert("작성자만 수정가능합니다");
	}else{
		location.href="updateboard.do?id="+id+"&seq="+seq;
	}
}
function showPopup() {
	...
	<div class="bbody">
		...
		<div class="buttons">
			<table class="resp_table shadow-lg">
				...
				<tr>
					<th>식당이름</th>
					<td class="td3">${title}</td>
				</tr>
				<tr>
					<th>내용</th>
					<td class="td4"><textarea rows="10" cols="60" readonly="readonly">${content}</textarea></td>
				</tr>
				<tr>
					<td colspan="2" style="padding-bottom: 15px;text-align: center;">
						<button  type="button" class="btndeco" onclick="update(${seq}, ${id })">수정</button>
						<button class="btndeco" onclick="location.href='delreview.do?seq=${seq}'" ;>삭제</button>
						<button class="btndeco" onclick="location.href='review.do'">목록</button>
					</td>
				</tr>
			</table>
				</div>
